...
class BootBot extends EventEmitter {
  ...
  _handlePassThreadControlEvent(event) {
    this._handleEvent('pass_thread_control', event);
  }

  _handleRequestThreadControlEvent(event) {
    this._handleEvent('request_thread_control', event);
  }
    ...
    data.entry.forEach((entry) => {
      ...
      entry.messaging.forEach((event) => {
        ...
        if (event.optin) {
          ...
          this._handlePassThreadControlEvent(event);
        } else if (event.request_thread_control) {
          this._handleRequestThreadControlEvent(event);
